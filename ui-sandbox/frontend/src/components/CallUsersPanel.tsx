...
export function CallUsersPanel({ className }: { className?: string }) {
  ...
  const currentUsername = auth?.user?.username as string | undefined;
  ...
  const hasSelectedCallee = Boolean(calleeId);
  ...
  return (
    <aside
      className={cn(
        "overflow-hidden rounded-2xl border border-ui-border bg-ui-sidebar",
        className
      )}
    >
